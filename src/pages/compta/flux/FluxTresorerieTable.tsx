...
export const FluxTresorerieTable: React.FC<FluxTableProps> = ({ lignes, formatMontant }) => {
  return (
    <div className="border border-navy-100 dark:border-navy-800 rounded-xl overflow-hidden shadow-xs bg-white dark:bg-navy-950 w-full">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-navy-50 dark:divide-navy-800 text-xs text-navy-700 dark:text-navy-300">
            {lignes.length === 0 ? (
              <tr>
                <td colSpan={3} className="text-center p-10 text-navy-400 italic">
              ...
              lignes.map((l, index) => {
                ...
                return (
                  <tr 
                    ...
                    className={`hover:bg-navy-50/10 dark:hover:bg-navy-800/10 transition-colors ${
                      isMasseMacro ? 'bg-slate-100/60 dark:bg-navy-900/40 font-black text-navy-900 dark:text-navy-50 border-y border-navy-200 dark:border-navy-800' : ''
                    }`}
                  >
                    <td className="p-2.5 pl-5" style={{ paddingLeft: `${20 + l.niveau * 16}px` }}>
                      <span className={isMasseMacro ? 'uppercase tracking-tight text-[11px]' : 'font-medium'}>
                        {l.intitule}
                      </span>
                    </td>
                    <td className={`p-2.5 text-right font-mono font-semibold ${isMasseMacro ? 'text-navy-900 dark:text-navy-50' : 'text-slate-700 dark:text-navy-300'}`}>
                      {formatMontant(l.montantN)}
                    </td>
                    <td className="p-2.5 text-right font-mono text-navy-400 dark:text-navy-500 pr-5">
