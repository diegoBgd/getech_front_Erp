            ...
            <div className="overflow-x-auto">
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-navy-50 dark:divide-navy-800 text-xs text-navy-700 dark:text-navy-300">
                        {/* Le point d'interrogation et la liste de repli empêchent le crash de Vite/React */}
                        {(lignes && Array.isArray(lignes) ? lignes : []).map((l, i) => (
                            <tr key={i} className={`hover:bg-navy-50/10 dark:hover:bg-navy-800/10 ${l.niveau === 0 ? 'font-bold bg-navy-50/10 dark:bg-navy-900/10 text-navy-900 dark:text-navy-50' : ''}`}>
                                ...
                                <td className="p-3 text-right font-mono">{formatMontant(l.brutN)}</td>
                                <td className="p-3 text-right font-mono text-rose-600 dark:text-rose-400">{formatMontant(l.amortissementN)}</td>
                                <td className="p-3 text-right font-mono text-navy-900 dark:text-navy-50">{formatMontant(l.netN)}</td>
                                <td className="p-3 text-right font-mono text-navy-400 dark:text-navy-500 pr-4">{formatMontant(l.netN1)}</td>
                            </tr>
                        ))}
                    </tbody>
